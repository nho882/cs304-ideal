...
def searchBar():
  if request.method == 'POST': 
    ...
    if company!="": #if nonempty, then fetch row from database
      curs.execute('select name from companies where name = %s', ('%'+company+'%',))
      row = curs.fetchone()
      if row is not None: #Returns redirect if nonempty
      	return redirect(url_for('display.html', company_name = row['name']))
...
def display_review(company_name=None, terms=None, identity=None):
